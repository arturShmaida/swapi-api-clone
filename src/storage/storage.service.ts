...
import { S3Client, PutObjectCommand, PutObjectCommandInput, GetObjectCommand, GetObjectCommandInput, DeleteObjectCommandInput, DeleteObjectCommand } from "@aws-sdk/client-s3";
...
export class StorageService {
    ...
    async getEnitiesWithSignedUrl(imageEntities: Image[]) {
        ...
        try {
            for (let entity of imageEntities) {
                const param: GetObjectCommandInput = {
                    Bucket: bucketName,
                    Key: entity.name
                }
                const command = new GetObjectCommand(param);
                entity.url = await getSignedUrl(s3, command, { expiresIn: IMAGE_URL_TIME_TO_LIVE });
            }
        } catch (error) {
            throw error;
